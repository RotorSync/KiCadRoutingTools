...
# build zone index: (net_id, layer) -> list of polygons
from collections import defaultdict
zone_index = defaultdict(list)
for z in pcb.zones:
    if z.net_id == 0:
        continue  # keepout / no-net
    zone_index[(z.net_id, z.layer)].append(z.polygon)

# replicate metric_stubs dangling detection
tol = 1e-3
seg_endpoints = []
...
print("dangling endpoints (net, layer):")
for (x, y, nid, layer) in dangling:
    ...
    print(f"  ({x:.3f}, {y:.3f}) net={nid}({name}) layer={layer} in_zone={in_zone}")
