...
class MovieRecorder:
    ...
    def _snapshot(self, name):
        ...
        (no board, or KiCad unreachable).

        The IPC plugin has no `pcbnew`: the board lives in the KiCad process and
        we reach it over the kipy socket. save_board_snapshot maps to
        SaveCopyOfDocument, so -- like the SWIG SaveBoard call this replaces --
        the user's own open file is never touched.
        """
        try:
