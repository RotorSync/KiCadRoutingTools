...
| "Re-route FAILED: no path found" for ripped nets | Retry budget exhausted | Increase `--max-iterations` (e.g. 1000000) — unless `KICAD_DYNAMIC_ITERATIONS=1` is set: the router then self-budgets (200k base, auto-extends to 1e7 while progressing; #529) and raising the flag is inert. If the log shows "dynamic iterations (#529): search extended to N" followed by failure, the net exhausted genuine progress — treat as a capacity problem, not a budget one |
