...
| "Re-route FAILED: no path found" for ripped nets | Search space exhausted | NOT a budget problem: the router self-budgets (#529, default on — auto-extends to 1e7 iterations while progressing), so do not suggest raising `--max-iterations`. A "dynamic iterations (#529): search extended to N" line followed by failure means genuine progress ran out — escalate capacity: `--max-ripup`, clearance/track toward fab floor, or layers. "dynamic iterations (#625): ... static caps" lines mean the run already wasted its failed-extension budget on that net — the same capacity verdict, reached cheaper; never counter them by raising budgets. (`KICAD_DYNAMIC_ITERATIONS=0` restores legacy static caps for A/B only) |
