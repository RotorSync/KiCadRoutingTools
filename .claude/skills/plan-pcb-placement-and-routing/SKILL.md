...
Measured, one such file took a board's `undersized` from **169 to 0**. There
is no per-net width floor to pass alongside it -- `--track-width-floor` was
removed in 53a5a16e -- so a width clause is carried by the fab-override file
and verified afterwards on the emitted copper. Check `min_clearance_used` in
the `JSON_SUMMARY` too: it is the only place a floor that was silently
loosened shows up.
