...
- **Also check `drc_grazes` (even when `failed == 0`).** The summary's
  ...
  **ceiling over every class** (`--clearance` caps each net at `min(its class,
  --clearance)`). On `check_drc` it is only the **global fallback**,
  ...
  or use `--clearance-margin` (default 0.05).
...
1c. **After ALL fanouts are done — once, not per-BGA — run
   ...
   collisions (#130) before signal routing. The pass is board-global (it reads
   every via and every BGA), so one late run sees everything; running it
   per-BGA compounds cap displacement (each run re-seeds at the already-moved
   position) and changes what later fanouts route around (cap pads are escape
   obstacles). See Step 1c.
...
2. **Route the whole signal step at that width** (re-route everything, not just the
   ...
   needs 0.025: "sub-0.4" reads as excluding it, and measurement says
   otherwise; see 9.5's worked case) is the complementary
