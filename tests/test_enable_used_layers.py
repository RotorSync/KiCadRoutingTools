...
# A checked-in board with a canonical KiCad layer table. This used to point at
# kicad_files/fanout_starting_point.kicad_pcb, which is GITIGNORED -- it is
# generated by test_fanout_and_route.py, so these tests only passed on a
# checkout where that test had already run and left the artifact behind. On a
# fresh clone (or a git worktree) all five failed with FileNotFoundError, and
# run_all.py runs this test BEFORE the one that produces the file, so the
# suite could not heal itself either.
SRC = os.path.join(ROOT, "kicad_files", "cap_chain.kicad_pcb")
