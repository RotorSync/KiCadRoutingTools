...
# gitignored (.gitignore:45), so on a fresh clone it does not exist. Copying it
# unconditionally raised FileNotFoundError at MODULE scope, which during pytest
# collection is an error on this file -- the fresh-clone hazard #457 item 3 names
# (#190/#245/#432/#433, the same shape that was fixed for
# test_enable_used_layers.py by repointing it at a tracked board).
_FANOUT_SEED = os.path.join(REPO, "kicad_files", "fanout_starting_point.kicad_pcb")
if not os.path.exists(_FANOUT_SEED):
    print("  SKIP fanout rule checks -- kicad_files/fanout_starting_point.kicad_pcb "
          "is generated by test_fanout_and_route.py and is not present")
    _run_fanout_section = False
else:
    _run_fanout_section = True

with tempfile.TemporaryDirectory() as d2:
  if _run_fanout_section:
