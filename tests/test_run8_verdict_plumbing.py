...
ledger warning are.
  ...
  A10  route_disconnected_planes ripped nets to clear a plane corridor, failed
...
def test_a10_rip_casualties_reach_the_verdict():
    ...
    src = open(os.path.join(ROOT, 'route_disconnected_planes.py'),
               encoding='utf-8').read()
...
def _record(ledger, board, **kw):
    argv = [sys.executable, '-X', 'utf8', os.path.join(ROOT, 'converge.py'),
...
def test_converge_nags():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        code, out = _record(ledger, board, lever='lap 2',
                            argv=[os.path.join(ROOT, 'check_drc.py'),
                                  board, '--clearance', '0.1'])
        ...
        code, out = _record(ledger, board, lever='lap 3',
                            argv=[os.path.join(ROOT, 'place_seed.py'),
