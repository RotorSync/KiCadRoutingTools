...
# cwd=/repo
python3 -X utf8 route.py step2.kicad_pcb step3.kicad_pcb --nets '*' '!GND' '!+3V3' --no-bga-zones --clearance 0.09 --track-width 0.0762 --via-size 0.45 --via-drill 0.2 --hole-to-hole-clearance 0.2 --grid-step 0.05 --max-ripup 10 --max-iterations 1000000 --rip-existing-nets '/USB/*' /CLK --protect-nets '/DDR*' /XTAL_IN
# cwd=/repo
python3 -X utf8 route_planes.py step3.kicad_pcb step4.kicad_pcb --nets GND +3V3 --plane-layers In1.Cu In4.Cu --corridor-nets '/USB/*' --via-size 0.45 --via-drill 0.2 --track-width 0.09 --clearance 0.10 --hole-to-hole-clearance 0.2 --grid-step 0.05
# cwd=/repo
