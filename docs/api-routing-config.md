...
| `track_width_floor` | `None` | Global width floor (mm): the router may not neck a route below it — the net FAILS honestly instead (`--track-width-floor`). A single scalar, not per-net; in a call scoped to one net's group it acts as that group's floor (the only place it can hold a spec width without pinning the whole board) |
