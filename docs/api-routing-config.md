...
*(There is no `track_width_floor` field and no `--track-width-floor` flag. Both were REMOVED in 53a5a16e, along with the `--net-layers` and `--protect-nets` FLAGS -- those two were never config fields. Necking is floored at the FAB minimum instead — `diff_pair_routing._neck_pair_partner_grazes` via `_fab_track_floor` — so a spec width above that floor is not held by the config at all: carry it in a `--fab-overrides` file and verify it on the emitted copper.)*
