"""Refine around alpha=0 fw=4 fp=1; also try fp=0.5."""
...
def fitness(params):
    ...
    for b in TRAIN:
        ...
        for W in total_w:
            ...
            s+=total_w[W]*sum(1 for v in vals if v>=0.8)/len(vals)
        per[b.split("/")[-1]]=s
        score+=s
...
combos=[]
for al in [0.0]:
    for fw in [2.0, 4.0, 6.0]:
        for fp in [0.5, 1.0]:
            combos.append({"alpha":al,"fidelity_weight":fw,"fidelity_power":fp})
results=[]
for params in combos:
    t0=time.time()
    fit,per=fitness(params)
    results.append({"params":params,"fit":fit,"per":per})
    print(f"[{time.time()-t0:.0f}s] {params} fit={fit:.4f} per={ {k:f'{v:.3f}' for k,v in per.items()} }", flush=True)
best=max(results,key=lambda r:r["fit"])
print("BEST:",best["params"],best["fit"])
with open("/home/austin/krt_work/planning_lab/mini_search2.json","w") as f:
    json.dump(results,f)
