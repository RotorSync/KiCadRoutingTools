...
**Important:** When making changes to the Rust router, bump the version in `rust_router/Cargo.toml` and update the version history in `rust_router/README.md`. The release triple is `rust_router/Cargo.toml` + `/VERSION` + `metadata.json` — keep them aligned. **The crate is 0.20.1 and the 0.20.1 binaries are NOT yet published** (latest release asset is v0.20.0), so a plain `python3 build_router.py` would download the older 0.20.0 asset and `startup_checks` rejects the mismatch — **use `--from-source` until the 0.20.1 binaries ship.** (0.20.1 removes the `block_vias` parameter from `add_stub_proximity_costs_batch`, so the 0.20.0 binary is API-incompatible with current Python besides the version gate.) Once matching binaries are published, plain builds work again and `--from-source` is only needed while testing local Rust changes.
