#!/usr/bin/env python3
"""Re-score SI-enforcement corpus at CURRENT defaults (R0.8/C0.1).

Replays the exact faithful chain (aggressors -> victims -> rest) from the
original si_corpus_ab logs for both arms, with fresh output paths.
Usage: si_corpus_rescore.py <board_key>
"""
...
STEPS = ["a", "v", "n"]
ARMS = [("off", {"KICAD_SI_ENFORCE": "0"}),
        ("on", {"KICAD_SI_ENFORCE": "1"})]  # ON = current defaults, no knob overrides


def parse_cmd(logpath):
    with open(logpath) as f:
        line = f.readline()
    # CMD: python /path/route.py <input> --output <out> --nets ... (shell-quoted)
    m = re.match(r'CMD:\s+(.*)$', line)
    if not m:
        raise RuntimeError(f"cannot parse CMD from {logpath}: {line!r}")
    toks = shlex.split(m.group(1))
    # toks: [python, route.py, input, --output, out, ...rest]
    inp = toks[2]
    out = None
    rest = []
    i = 3
    ...
    return inp, out, rest  # input, output, rest-args
...
def run_board(key):
    ...
    # Parse the three step commands from the OFF logs.
    steps = {}
    for s in STEPS:
    ...
    for arm, env in ARMS:
        ...
        for s in STEPS:
            _, _, rest = steps[s]
            out_path = os.path.join(odir, f"{arm}_{s}.kicad_pcb")
            if os.path.exists(out_path):
                print(f"[{key} {arm} {s}] exists, skip", flush=True)
                cur_inp = out_path
                continue
            if free_gb() < 8:
                print(f"[{key} {arm} {s}] FREE MEM {free_gb()}G < 8G, waiting...", flush=True)
                while free_gb() < 8:
                    time.sleep(30)
            ...
            full_env.update(env)
            logp = os.path.join(odir, f"{arm}_{s}.log")
            print(f"[{key} {arm} {s}] start {time.strftime('%H:%M:%S')}", flush=True)
            t0 = time.time()
            with open(logp, "w") as lf:
                r = subprocess.run(["nice", "-n", "19"] + cmd,
                                   env=full_env, stdout=lf, stderr=subprocess.STDOUT)
            dt = time.time() - t0
            print(f"[{key} {arm} {s}] exit={r.returncode} {dt:.1f}s", flush=True)
            if r.returncode != 0:
                print(f"[{key} {arm} {s}] FAILED rc={r.returncode}", flush=True)
                sys.exit(1)
            cur_inp = out_path
